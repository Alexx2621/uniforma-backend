CREATE TABLE `BordadoDetallePedidoProduccion` (
  ...
  INDEX `BordadoDetallePedidoProduccion_detalleId_idx`(`detalleId`),
  INDEX `BordadoDetallePedidoProduccion_estado_idx`(`estado`),
  PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

ALTER TABLE `BordadoDetallePedidoProduccion`
  ADD CONSTRAINT `BordadoDetallePedidoProduccion_detalleId_fkey`
  FOREIGN KEY (`detalleId`) REFERENCES `DetallePedidoProduccion`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

INSERT INTO `BordadoDetallePedidoProduccion` (
  `detalleId`,
  `monto`,
  `color`,
  `tamano`,
